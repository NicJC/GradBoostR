...
// [[Rcpp::export]]
List rf_fit_fast(const arma::mat& X,
                 const arma::vec& y,
                 int n_trees = 200,
                 int max_depth = 5,
                 int mtry = 3) {
  ...
  Rcpp::List out_forest(forest.size());
...
double predict_tree(const arma::rowvec& x,
  ...
  if (x[node.feature] <= node.threshold)
    return predict_tree(x, nodes, node.left);
  else
    return predict_tree(x, nodes, node.right);
}

// [[Rcpp::export]]
arma::vec rf_predict_fast(const arma::mat& X, List forest_list) {
  ...
  for (int t = 0; t < n_trees; ++t) {
    ...
    for (int i = 0; i < n; ++i)
      out[i] += predict_tree(X.row(i), nodes);
  }
